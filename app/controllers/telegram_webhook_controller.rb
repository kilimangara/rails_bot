...
Я собираю в себе полезные сервисы и услуги для студентов МИФИ! '.freeze

  INTRO_SHAURMA = 'Оформи заказ и забери его без очередей и ожидания!'.freeze
  ...
  def send_notify(order, with_delivery)
    ...
    text << "Клиент: #{order.user.name} #{order.user.phone}\n"
